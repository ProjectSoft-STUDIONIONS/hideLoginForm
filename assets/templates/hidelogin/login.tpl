...
<html>
	<head>
		...
		<style>
			...
			body {background-image: url('/assets/templates/hidelogin/images/evo.jpg'); background-position:50% 50%; background-repeat: no-repeat; background-size: cover; }
			...
			/* mainloader */
			#mainloader { position: absolute; z-index: 50000; top: 0; left: 0; width: 100%; height: 100%; text-align: center; vertical-align: middle; padding: 15% 0 0 0; background-color: rgba(255, 255, 255, 1); opacity: 0; visibility: hidden; -webkit-transition-duration: 0.3s; transition-duration: 0.3s }
	...
	<body class="[+manager_theme_style+]">
		<div class="page">
			<div class="tab-page loginbox">
				<form class="container container-body" method="post" name="loginfrm" id="loginfrm" action="processors/login.processor.php">[+OnManagerLoginFormPrerender+]
					<div class="form-group text-center"><a class="logo" href="../" title="[(site_name)]"><img id="logo" src="media/style/[(manager_theme)]/images/misc/login-logo.png" alt="[(site_name)]"></a></div>
					<div class="form-group">
						<label class="text-muted text-muted" for="username">[+username+]</label>
						<input class="form-control" id="username" type="text" name="username" tabindex="1" value="[+uid+]">
					</div>
					<div class="form-group">
						<label class="text-muted" for="password">[+password+]</label>
						<input class="form-control" id="password" type="password" name="password" tabindex="2" value="">
					</div>
					<div class="clearfix">
						<div class="caption">[+login_captcha_message+]</div>
						<p>[+captcha_image+]</p>[+captcha_input+]
					</div>
					<div class="form-group">
						<label class="text-muted" for="rememberme">
							<input class="checkbox" id="rememberme" type="checkbox" name="rememberme" value="1" [+remember_me+]>[+remember_username+]
						</label>
						<button class="btn btn-success float-xs-right" id="submitButton" type="submit" name="submitButton">[+login_button+]</button>
					</div>[+OnManagerLoginFormRender+]
				</form>
			</div>
			<div class="copyrights">
				<p class="loginLicense"></p>
				<div class="gpl">&copy; 2005-2018 by the <a href="http://evo.im/" target="_blank">EVO</a> . <strong>EVO</strong>&trade; is licensed under the GPL.
				</div>
			</div>
		</div>
